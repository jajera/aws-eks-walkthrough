...
The AWS CLI is how you authenticate to <Tooltip term="aws" /> from your machine. Terraform uses the same credential chain to create resources, and `aws eks update-kubeconfig` uses it to write your kubeconfig after the cluster exists. Getting this right first avoids authentication errors in every subsequent step.
...
2. **Configure AWS access.** <Badge text="Required" variant="tip" size="small" />
   ...
   <Tabs syncKey="credential-type">
     <TabItem label="IAM user (access keys)">
      You need the Access Key ID and Secret Access Key you created in the [AWS Account](/aws-eks-walkthrough/foundations/prerequisites/aws-account/) step.
     ...
     <TabItem label="IAM Identity Center (SSO)">
      If you completed the [IAM Identity Center](/aws-eks-walkthrough/foundations/prerequisites/iam-identity-center/) step, your profile is already configured. Log in:
...
Continue to [Terraform](/aws-eks-walkthrough/foundations/prerequisites/terraform/) to install the infrastructure-as-code tool.
